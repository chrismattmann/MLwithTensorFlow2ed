...
def model(x, y, keep_prob=1.0):  #VGG-Face-Lite   
    ...
    cost = tf.reduce_mean(cost + beta * regularizer, name='cost') #L2 regularization
...
all_image_paths = []
with open('input/shuffle.list') as f:
    all_image_paths = ["/tmp/fs_801937/"+line.strip() for line in f]
'''
...
target_learning_rate = starter_learning_rate * 4
global_step = tf.train.get_or_create_global_step()
#learning_rate = tf.compat.v1.train.exponential_decay(starter_learning_rate,global_step, 100000, 0.96, staircase=True)
warmup_steps = 401*5
learning_rate = warmup_exponential_decay(starter_learning_rate, target_learning_rate, global_step, warmup_steps, 100000, 0.96, staircase=True)
...
val_image_batch, val_label_batch = val_iter.get_next()

acc, cost = model(image_batch, label_batch, keep_probability)
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=0.9, beta2=0.999, epsilon=0.1)
#optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.9)
train_op = hvd.DistributedOptimizer(optimizer)
...
init_hook = _DatasetInitializerHook()
...
hooks = [
        ...
        #tf.train.StopAtStepHook(last_step=epochs*len(train_labels) // (BATCH_SIZE*hvd.size())),
        tf.train.StopAtStepHook(last_step=401000),

        tf.train.LoggingTensorHook(tensors={'step': global_step, 'accu': acc, 'loss': cost},
                                   every_n_iter=10),
        init_hook
    ]

steps_per_epoch = len(train_labels)//(BATCH_SIZE*hvd.size())
...
with tf.train.MonitoredTrainingSession(checkpoint_dir=checkpoint_dir,
    ...
    #for i in range(epochs*steps_per_epoch):
        try:
            ...
            epoch = batch_count // steps_per_epoch
            print("Epoch %d fininishes" % epoch)
            sess.run(iter.initializer)
            continue
        '''
        epoch = batch_count // steps_per_epoch 
        #print("Epoch %d, Step %d" % (epoch, batch_count))
        if epoch == next_epoch and epoch % ckpt_interval == 0:
            next_epoch += ckpt_interval
            batch_num = 0
            sum_acc = 0.0
            sum_loss = 0.0
            while batch_num*BATCH_SIZE*hvd.size() < len(test_labels):
                val_image_batch, val_label_batch = sess._tf_sess().run(val_iter_op)
                v_acc, v_loss = sess.run([acc, cost], feed_dict={x:val_image_batch, y:val_label_batch, keep_prob:1.0})
                sum_acc += v_acc
                sum_loss += v_loss
                batch_num += 1
                print("Epoch %d, Step %d: Val_Accuracy %f Val_Loss %f " % (epoch, batch_num, v_acc, v_loss))
                sys.stdout.flush()
            print("Epoch %d Validation Accuracy %f Validation Loss %f" % (epoch, sum_acc/batch_num, sum_loss/batch_num))
            sys.stdout.flush()
        '''
